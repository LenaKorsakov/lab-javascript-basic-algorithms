...
let newCleanPhrase = phraseToCheck.replace(/[^A-Z0-9]+/gi, "").toUpperCase();
let checkPalindrome = true;

for (let i = 0; i < phraseToCheck.length / 2; i++) {
  if (newCleanPhrase[i] !== newCleanPhrase[newCleanPhrase.length - 1 - i]) {
    checkPalindrome = false;
    break;
  }
}

console.log(checkPalindrome ? "it is a palindrome" : "it's not palindrome");
